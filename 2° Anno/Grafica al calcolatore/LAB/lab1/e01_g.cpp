...
int main(int argc, char const *argv[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(800, 800, "cg-lab", NULL, NULL);
	if(!window)
	{
		glfwTerminate();
		exit(EXIT_FAILURE);
	}
	glfwMakeContextCurrent(window);    //per OpenGL contex active
	...
	glfwSetKeyCallback(window, key_callback);  //Setta una funzione che viene avviata
                                               // per questa finestra e li dici cosa fare
	...
	while(!glfwWindowShouldClose(window))
	{
		...
		glfwSwapBuffers(window); // Scambia i buffer per la visualizzazione
		glfwPollEvents();        // controllo eventi in input
	}

	destroy_vao(); check(__LINE__);
	destroy_shader(); check(__LINE__);

	glfwDestroyWindow(window); //Chiudere la finestra
